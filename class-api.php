...
class Teste
{
    ...
    {
        if (isset($this->api_refresh_token) && is_array($this->api_refresh_token)) {
            ...
            return true;
        } elseif (isset($this->api_refresh_token) &&  $this->api_refresh_token) {
    ...
    private function curl($endpoint, $method = 'GET', $data = false, $retry = false)
    {
        ...
        if ((isset($result->error) && isset($this->api_refresh_token) && !$retry) or (isset($result->message) && !$retry)) {
